...
nominalFrameRate = Screen('NominalFrameRate', window);			% 10 seconds ~ + 500;
...
stim = getStim;								% === Gets stimulus ===

moviefile = [pwd filesep 'stim1.avi'];
...
for i = 1:length(stim)	% presSecs)

    % Convert our current number to display into a string
    numberString = stim{i};		% num2str(presSecs(i));
	
    % Draw our number to the screen
    DrawFormattedText(window, numberString, 'center', 'center', black);
